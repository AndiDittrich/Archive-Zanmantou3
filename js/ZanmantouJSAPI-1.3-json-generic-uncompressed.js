...
/* Includes: JSON Library http://www.JSON.org/json2.js *//**
 *  JSON Library
...
// If the text is not JSON parseable, then a SyntaxError is thrown.
	...
	// If the JSON object does not yet have a encode alias, give it one.
    if (typeof JSON.encode !== 'function') {
		...
		// If the JSON object does not yet have a decode alias, give it one.
...
/**
 * 	Zanmantou3 WebAudioVideoPlayer - Javascript API
 *	
 ...
 */
var a3non_zanmantou_instances = new Array();
...
var a3non_zanmantou_callback = function(instanceID, event, args){
	for (var i = 0;i<a3non_zanmantou_instances.length;i++){
		if (a3non_zanmantou_instances[i][0] == instanceID){
			a3non_zanmantou_instances[i][1].fireEvent(event, args);
		}
	}
};
...
/**
 * FlashReady Lowlevel Event Callback Singleton
 */
var a3non_zanmantou_flashready_callback = function(){
	if (!a3non_zanmantou_flashready_istriggered){
		...
		a3non_zanmantou_flashready();
	}	
};

/**
 * Zanmantou3 Javascript API 
 * @param {String|Object} id
 * @param {Object} options
 */
var Zanmantou = function(id){
	this.instanceid = document.getElementById(id);

	// store instance
	a3non_zanmantou_instances.push(new Array(id, this));
...
Zanmantou.prototype.addEvent = function(event, fn){
		this.events.push(new Array(event, fn));
	};

Zanmantou.prototype.removeEvent = function(event, fn){
...
Zanmantou.prototype.fireEvent = function(event, arg){
		for (var i = 0;i<this.events.length;i++){
...
Zanmantou.prototype.start = function(){
		this.instanceid.z_start();
	};

Zanmantou.prototype.jumpAndStart = function(index){
		this.stop();
		this.setMediaIndex(index);
		this.start();
	};

Zanmantou.prototype.seek = function(pos){
		this.instanceid.z_seek(pos);
	};

Zanmantou.prototype.play = function(url, type, pos){
...
Zanmantou.prototype.pause = function(){
...
Zanmantou.prototype.prev = function(){
		this.instanceid.z_prev();
	};

Zanmantou.prototype.shuffle = function(enable){
		this.instanceid.z_shuffle(enable);
	};

Zanmantou.prototype.repeat = function(enable){
		this.instanceid.z_repeat(enable);
	};

Zanmantou.prototype.repeatall = function(enable){
		this.instanceid.z_repeatall(enable);
	};

Zanmantou.prototype.smoothing = function(enable){
		this.instanceid.z_smoothing(enable);
	};

Zanmantou.prototype.mute = function(){
		this.instanceid.z_mute();
	};

Zanmantou.prototype.unmute = function(){
		this.instanceid.z_unmute();
	};

Zanmantou.prototype.setVolume = function(volume){
...
Zanmantou.prototype.setPan = function(balance){
		this.instanceid.z_setPan(balance);
	};

Zanmantou.prototype.getPan = function(){
		return this.instanceid.z_getPan();
	};

Zanmantou.prototype.setLRTransform = function(ll, lr, rr, rl){
		this.instanceid.z_setLRTransform(ll, lr, rr, rl);
	};

Zanmantou.prototype.getWave = function(){
		return JSON.decode(this.instanceid.z_getWave());
	};

Zanmantou.prototype.getSpectrum = function(){
		return JSON.decode(this.instanceid.z_getSpectrum());
	};

Zanmantou.prototype.getMediaList = function(){
		return JSON.decode(this.instanceid.z_getMediaList());
	};

Zanmantou.prototype.setMediaList = function(list){
		this.instanceid.z_setMediaList(JSON.encode(list));
	};

Zanmantou.prototype.setMediaIndex = function(i){
		this.instanceid.z_setMediaIndex(i);
	};

Zanmantou.prototype.getMediaIndex = function(){
		return this.instanceid.z_getMediaIndex();
	};

Zanmantou.prototype.addMediaItem = function(index, data){
		this.instanceid.z_addMediaItem(index, JSON.encode(data));
	};

Zanmantou.prototype.removeMediaItem = function(index){
		this.instanceid.z_removeMediaItem(index);
	};

Zanmantou.prototype.isPlaying = function(){
		return this.instanceid.z_isPlaying();
	};

Zanmantou.prototype.getMediaLength = function(){
		return this.instanceid.z_getMediaLength();
	};

Zanmantou.prototype.getMediaTime = function(){
		return this.instanceid.z_getMediaTime();
	};

Zanmantou.prototype.getMediaProgress = function(){
		return this.instanceid.z_getMediaProgress();
	};

Zanmantou.prototype.getMediaLoadingProgress = function(){
		return this.instanceid.z_getMediaLoadingProgress();
	};

Zanmantou.prototype.getFPS = function(){
		return this.instanceid.z_getFPS();
	};

Zanmantou.prototype.getVisionLightColors = function(){
		return JSON.decode(this.instanceid.z_getVisionLightColors());	
	};

Zanmantou.prototype.enableVisionLight = function(e){
